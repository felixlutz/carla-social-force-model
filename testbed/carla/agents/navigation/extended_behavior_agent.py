...
class ExtendedBehaviorAgent(ExtendedBasicAgent):
    """
    ...
    def __init__(self, vehicle, behavior='normal'):
        ...
        super(ExtendedBehaviorAgent, self).__init__(vehicle)
    ...
    def pedestrian_avoid_manager(self, waypoint):
        ...
        if self._direction == RoadOption.CHANGELANELEFT:
            walker_state, walker, distance = self._vehicle_obstacle_detected(walker_list, max(
                self._behavior.min_proximity_threshold, self._speed_limit / 2), up_angle_th=90, lane_offset=-1, pedestrians=True)
        elif self._direction == RoadOption.CHANGELANERIGHT:
            walker_state, walker, distance = self._vehicle_obstacle_detected(walker_list, max(
                self._behavior.min_proximity_threshold, self._speed_limit / 2), up_angle_th=90, lane_offset=1, pedestrians=True)
        else:
            walker_state, walker, distance = self._vehicle_obstacle_detected(walker_list, max(
                self._behavior.min_proximity_threshold, self._speed_limit / 3), up_angle_th=90, pedestrians=True)
