...
class ExtendedBasicAgent(object):
    """
    ...
    def __init__(self, vehicle, target_speed=20, opt_dict={}):
        ...
        self._ignore_pedestrians = False
        ...
        if 'ignore_pedestrians' in opt_dict:
            self._ignore_pedestrians = opt_dict['ignore_pedestrians']
    ...
    def ignore_pedestrians(self, active=True):
        """(De)activates the checks for stop signs"""
        self._ignore_pedestrians = active
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0, pedestrians=False):
        """
        Method to check if there is a vehicle in front of the agent blocking its path.
            ...
            :param max_distance: max freespace to check for obstacles.
                If None, the base threshold value is used
        """
        if self._ignore_vehicles and not pedestrians or self._ignore_pedestrians and pedestrians:
            return (False, None, -1)
        ...
        for target_vehicle in vehicle_list:
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                if pedestrians:
                    lane_ids = [ego_wpt.lane_id, ego_wpt.lane_id + 1, ego_wpt.lane_id -1]
                else:
                    lane_ids = [ego_wpt.lane_id + lane_offset]

                if target_wpt.road_id != ego_wpt.road_id or target_wpt.lane_id not in lane_ids:
                    next_wpt = self._local_planner.get_incoming_waypoint_and_direction(steps=3)[0]
                    if not next_wpt:
                        continue

                    if pedestrians:
                        next_lane_ids = [next_wpt.lane_id, next_wpt.lane_id + 1, next_wpt.lane_id -1]
                    else:
                        next_lane_ids = [next_wpt.lane_id + lane_offset]

                    if target_wpt.road_id != next_wpt.road_id or target_wpt.lane_id not in next_lane_ids:
                        continue
