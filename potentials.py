...
class PedPedPotential(object):
    ...
    def grad_r_ab(self, state, delta=1e-3):
        ...
        rdx = self.value_r_ab(r_ab + dx, speeds, desired_directions)
        rdy = self.value_r_ab(r_ab + dy, speeds, desired_directions)
        rdz = self.value_r_ab(r_ab + dz, speeds, desired_directions)
        dvdx = (rdx - v) / delta
        dvdy = (rdy - v) / delta
        dvdz = (rdz - v) / delta
